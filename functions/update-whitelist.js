  ...
  try {
    ...
    if (action === 'add') {
      ...
      if (!groupIds.includes(groupId)) {
        ...
        return { statusCode: 200, body: JSON.stringify({ message: 'Group ID not in whitelist' }) };
      }
    ...
    try {
      await octokit.repos.createOrUpdateFileContents({
        ...
        repo,
        path,
        message: `${action === 'add' ? 'Add' : 'Remove'} group ID ${groupId}`,
    ...
    return { statusCode: 200, body: JSON.stringify({ message: `Group ID ${action === 'add' ? 'added to' : 'removed from'} whitelist` }) };
  } catch (error) {
